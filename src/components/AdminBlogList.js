                ...
                <div style={{ marginTop: isMobile ? '32px' : '119px', flexDirection: 'column', marginBottom: '104px', display: isMobile ? 'none' : 'flex' }}>
                    {blogs.map((blog) => (
                        <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '56px' }}>
                            <div style={{ flex: 1,display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }} className='container'>
                                <img className='image' onClick={() => { handleRedirect(blog._id) }} src={blog.image} style={{ cursor: 'pointer', width: '100%', height: 'auto', maxHeight: 284, maxWidth: 504, transition: "0.5s ease",backfaceVisibility: 'hidden' }} />
                                <div onClick={() => { handleRedirect(blog._id) }} className={"middle"}>
                ...
                <div style={{ marginTop: isMobile ? '32px' : '119px', flexDirection: 'column', marginBottom: '104px', display: isMobile ? 'flex' : 'none' }}>
                    {blogs.map((blog) => (
                        <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: isMobile ? '16px' : '56px' }}>
                            <div style={{ flex: 1 }}>
                                <img onClick={() => { handleRedirect(blog._id) }} src={blog.image} style={{ cursor: 'pointer', width: '100%', height: 'auto', maxHeight: 128, minHeight: 128 }} />
                            </div>
